...
class UserController extends AbstractController {
    ...
    public function logInUser(ApiAuthentification $apiAuthentification, UserPasswordHasherInterface $userPasswordHasherInterface,  Request $request, SerializerInterface $serializerInterface, UserRepository $userRepository, Messaging $messaging ):Response {
        ...
        try {
            ...
            if (!$json) {
                return $this->json(
                    ['Error' => 'Je Json es cide ou n\'existe pas.'],
                    400,
            ...
            if (empty($email) OR empty($password)) {
                return $this->json(
                    ['Error' => 'Toutes les données ne sont pas renseignées.'],
                    400,
                    ['Content-Type'=>'application/json','Access-Control-Allow-Origin' =>'*', 'Access-Control-Allow-Method' => 'GET'], 
                    []
                );
            ...
            if ($apiAuthentification->authentification($userPasswordHasherInterface ,$userRepository, $email, $password )) {
                ...
                if ($mailStatus != 'The mail has been sent') {
                    return $this->json(
                        ['Error' => 'Impossible d\'envoyer le mail de confirmation. Merci de réessayer plus tard.'],
                        500,
                        ['Content-Type'=>'application/json','Access-Control-Allow-Origin' =>'*', 'Access-Control-Allow-Method' => 'GET'], 
                        []
                    );
                ...
                return $this->json(
                    ['Success'=> 'Un email de double indentification vient d\'être envoyé à '.$user->getEmail()],
                    200, 
                    ['Content-Type'=>'application/json','Access-Control-Allow-Origin' =>'*', 'Access-Control-Allow-Method' => 'GET'],
                    []
                );
                ...
                return $this->json(
                    ['Error' => 'Connexion refusée : l\'identifiant et/ou le mot de passe n\'est pas correct'],
                    401,
                    ['Content-Type'=>'application/json','Access-Control-Allow-Origin' =>'*', 'Access-Control-Allow-Method' => 'GET'], 
                    []
                );
     ...
     public function logInValidation(string $id, string $token, Request $request, SerializerInterface $serializerInterface, UserRepository $userRepository, ApiAuthentification $apiAuthentification):Response {
         ...
         if ($checkToken != "Expired token" && $checkToken!= true) {
             return $this->json(
                 ['Error' => 'Token invalide. Connexion impossible.'],
                 498,
                 ['Content-Type'=>'application/json','Access-Control-Allow-Origin' =>'*', 'Access-Control-Allow-Method' => 'GET'], 
                 []
             );
         ...
         if ($checkToken === "Expired token") {
             return $this->json(
                 ['Error' => 'Token expiré, veuillez renouveller votre demande de connexion.'],
                 419,
                 ['Content-Type'=>'application/json','Access-Control-Allow-Origin' =>'*', 'Access-Control-Allow-Method' => 'GET'], 
                 []
             );
